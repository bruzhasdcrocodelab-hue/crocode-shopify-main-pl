  ...
  return elements.map((element, index) => {
    switch (element.type) {
      case "h2":
        return (
          <Fade direction="up" key={index}>
            <h2 className={options.h2ClassName}>
              {parseTextWithUnderline(
                element.content[0],
                options.underlinedClassName,
              )}
            </h2>
          </Fade>
        );
      case "h3":
        return (
          <Fade direction="up" key={index}>
            <h3 className={options.h3ClassName}>
              {parseTextWithUnderline(
                element.content[0],
                options.underlinedClassName,
              )}
        ...
        return (
          <ul key={index} className={options.listClassName}>
            {element.content.map((item, itemIndex) => (
              <Fade direction="down" key={itemIndex}>
                <li className={options.listItemClassName}>
      ...
      default:
        return (
          <Fade direction="down" key={index}>
            <p className={options.paragraphClassName}>
              {parseTextWithUnderline(
                element.content[0],
                options.underlinedClassName,
              )}
